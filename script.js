...
.panel-left textarea,.panel-right textarea{
  width:100%;height:110px;padding:10px;font-size:13px;
  border:1px solid #dadce0;border-radius:4px;resize:vertical;
}
#absentWords{
  display:none;padding:10px 14px;background:#fff3cd;
  border-bottom:1px solid #dadce0;font-size:13px;color:#856404;line-height:1.6;
}
#absentWords strong{display:block;margin-bottom:4px}
#absentWords .absent-list{font-weight:600;word-break:break-word}
#absentWords .found-msg{
  color:#155724;background:#d4edda;
  border-radius:4px;padding:6px 10px;display:inline-block;
}
#sheetWrap{flex:1;overflow:auto;background:#fff}
table{border-collapse:collapse;table-layout:fixed;background:#fff}
th{
  border:1px solid #e0e0e0;background:#f8f9fa;color:#5f6368;
  font-weight:500;text-align:center;height:32px;min-width:100px;
  width:100px;font-size:12px;position:sticky;top:0;z-index:20;
}
.row-head{
  min-width:50px;width:50px;left:0;z-index:30;
  position:sticky;background:#f8f9fa;
}
td{
  border:1px solid #e0e0e0;width:100px;height:32px;
...
td.highlighted{
  background-color:#34ce57!important;color:#141414!important;
}
td.reading{
  background-color:#34ce57!important;color:#141414!important;
...
.col-select option{background:#fff;color:#202124}
...
<header>
  ...
  <span class="hint">Click any word to highlight all occurrences \u2022 Paste vocab to seekout</span>
</header>

<div id="toolbar">
  <label>Speed <input id="speed" type="range" min="0.5" max="2" step="0.1" value="1"></label>
  <label>Repeat Cell <input id="repeatCell" type="number" min="1" value="1" style="width:50px"></label>
  <label>Start <input id="startCell" type="text" placeholder="A1" style="width:55px"></label>
  <label>End <input id="endCell" type="text" placeholder="Z26" style="width:55px"></label>
...
<div id="inputPanel">
  <div class="panel-left">
    ...
    <textarea id="baseText" placeholder="Paste your text \u2014 Bible, Quran, Tao Te Ching, poems, essays. Sentences become rows, words become cells."></textarea>
    <button class="btn-green" onclick="uploadBaseText()">\u2B06 Upload Text</button>
  </div>
...
if("speechSynthesis" in window){
  ...
  tLoadVoices();setTimeout(tLoadVoices,250);setTimeout(tLoadVoices,1000);
}
...
function tSpeak(text,lang,rate){
  return new Promise(function(res){
    ...
    u.rate=rate||1;u.onend=res;u.onerror=res;
...
function initTablerTable(){
  ...
  h+='</tr><tr><th></th>';
  for(var c=0;c<TC;c++)h+='<th><select class="col-select"><option>Off</option><option>EN</option><option>IT</option><option>ES</option><option>FR</option><option>DE</option></select></th>';
  h+='</tr>';
...
function splitSentences(t){
  ...
  for(var i=0;i<lines.length;i++){
    ...
    var p=l.match(/[^.!?\\u3002\\uff01\\uff1f\\uff1b]*[.!?\\u3002\\uff01\\uff1f\\uff1b]+[\\s\\u00A0]*/g);
    if(p&&p.length){
      for(var j=0;j<p.length;j++){var s=p[j].trim();if(s)out.push(s);}
...
function tokenizeSent(s){
  var t=s.match(/[\\p{L}]+|[\\p{N}]+|[^\\s\\p{L}\\p{N}]+/gu);
  return t||[];
}
...
function uploadBaseText(){
  ...
  for(var i=0;i<units.length;i++){
    ...
    for(var j=0;j<tk.length&&j<TC;j++){
      var cell=tCell(i,j);
      if(cell){cell.textContent=tk[j];cell.setAttribute("data-word",tk[j].toLowerCase());}
    }
...
function startTablerReading(){
  ...
  var spd=parseFloat(document.getElementById("speed")?document.getElementById("speed").value:"1")||1;
  var rep=parseInt(document.getElementById("repeatCell")?document.getElementById("repeatCell").value:"1")||1;
  var s=tParseCell(document.getElementById("startCell")?document.getElementById("startCell").value:"A1")||{row:1,col:0};
  var e=tParseCell(document.getElementById("endCell")?document.getElementById("endCell").value:"Z26")||{row:tRows,col:TC-1};
  var rev=document.getElementById("reverse")?document.getElementById("reverse").checked:false;
  var rr=[],cr=[];
  ...
  (async function(){
    for(var ri=0;ri<rr.length;ri++){
      ...
      for(var ci=0;ci<cr.length;ci++){
        ...
        var lang=sel?sel.value:"Off";
        if(lang==="Off"||!txt.trim())continue;
...
function toggleTablerReader(){var b=document.getElementById("toolbar");b.style.display=b.style.display==="flex"?"none":"flex";}
...
tTable.addEventListener("keydown",function(e){
  ...
  if(e.key==="Tab"){
    e.preventDefault();
    var ci=-1,ch=tgt.parentElement.children;
    ...
    var nxt=null;
    if(ci>=TC){
      if(ri>=tTable.rows.length-1){addTablerRows(1);nxt=tTable.rows[tTable.rows.length-1].cells[1];}
...
function tUpdateLangs(){
  ...
  var unique={};
  for(var i=0;i<tVoices.length;i++){
    if(!tVoices[i].lang)continue;
    var k=tNorm(tVoices[i].lang);if(!unique[k])unique[k]=tVoices[i].lang;
  }
  var langs=Object.values(unique).sort();
  if(!langs.length)return;
  for(var c=1;c<sr.cells.length;c++){
    ...
    for(var i=0;i<langs.length;i++){
      var o=document.createElement("option");o.value=langs[i];o.textContent=langs[i];sel.appendChild(o);
    }
    sel.value=cur;
  }
}
setTimeout(tUpdateLangs,500);setTimeout(tUpdateLangs,1500);
